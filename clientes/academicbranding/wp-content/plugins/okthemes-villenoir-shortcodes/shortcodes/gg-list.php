...
if ( ! class_exists( 'WPBakeryShortCode_gg_List' ) ) {
	...
	class WPBakeryShortCode_gg_List extends WPBakeryShortCode {
	   ...
	   public function gg_list( $atts, $content = null ) { 
			 ...
			 //Start the insanity
			 $output = '';
			 $output .= "\n\t".'<div class="gg_list list_style_'.$list_style.' ' . ( ($list_border == "yes") ? 'list_border_bottom' : '' ). ' ' . esc_attr( trim( vc_shortcode_custom_css_class( $css ) ) ) . '">'.wpb_js_remove_wpautop( $newcontent ).'</div>'; 
			 //return the output
			 return $output;
