...
vc_map( array(
   ...
   "params" => array(
      ...
      array(
         ...
         "value" => array(esc_html__("Grid Fit rows", "okthemes-villenoir-shortcodes") => "fitRows"),
  ...
  function productCategoryCategoryAutocompleteSuggester( $query, $slug = false ) {
    ...
    if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
      foreach ( $post_meta_infos as $value ) {
        ...
        $data['label'] = esc_html__( 'Id', 'okthemes-villenoir-shortcodes' ) . ': ' .
                         $value['id'] .
                         ( ( strlen( $value['name'] ) > 0 ) ? ' - ' . esc_html__( 'Name', 'okthemes-villenoir-shortcodes' ) . ': ' .
                                                              $value['name'] : '' ) .
                         ( ( strlen( $value['slug'] ) > 0 ) ? ' - ' . esc_html__( 'Slug', 'okthemes-villenoir-shortcodes' ) . ': ' .
                                                              $value['slug'] : '' );
        $result[] = $data;
      }
    }
...
if ( !function_exists( 'productCategoryCategoryRenderBySlugExact' ) ) {
    function productCategoryCategoryRenderBySlugExact( $term ) {
        ...
        $term_slug_display = '';
        if ( ! empty( $term_sku ) ) {
        $term_slug_display = ' - ' . esc_html__( 'Sku', 'okthemes-villenoir-shortcodes' ) . ': ' . $term_slug;
        }

        $term_title_display = '';
        if ( ! empty( $product_title ) ) {
        $term_title_display = ' - ' . esc_html__( 'Title', 'okthemes-villenoir-shortcodes' ) . ': ' . $term_title;
        }

        $term_id_display = esc_html__( 'Id', 'okthemes-villenoir-shortcodes' ) . ': ' . $term_id;
