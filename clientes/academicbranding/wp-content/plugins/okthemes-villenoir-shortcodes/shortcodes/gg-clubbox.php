...
class WPBakeryShortCode_gg_clubbox extends WPBakeryShortCode {
   ...
   public function gg_clubbox( $atts, $content = null ) { 
        ...
        if ( strlen( $link ) > 0 && strlen( $url['url'] ) > 0 ) {
            $output .= "\n\t".'<div class="clearfix"></div><a class="club-link button btn-primary" href="' . esc_url( $url['url'] ) . '" title="' . esc_attr( $url['title'] ) . '" target="' . ( strlen( $url['target'] ) > 0 ? esc_attr( $url['target'] ) : '_self' ) . '">' . esc_attr( $url['title'] ) . '</a>';
        }
        $output .= "\n\t".'</div>';
		$output .= "\n\t".'</div>';
