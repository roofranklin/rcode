...
$extra_class = '';
$page_column_class = yoome_page_layout_columns_class($theme_options['ts_prod_cat_layout']);
...
yoome_breadcrumbs_title($show_breadcrumb, $show_page_title, woocommerce_page_title(false));

$top_filter_position = $theme_options['ts_top_filter_widget_area_position'];
$show_top_filter_widget_area = yoome_is_active_top_filter() && $top_filter_position != 'sidebar';
...
<div class="page-container <?php echo esc_attr($extra_class) ?>">
	...
	<?php if( $page_column_class['left_sidebar'] ): ?>
		...
		if( $show_sidebar_filter_widget_area ){
			...
			echo '<div class="widget-title-wrapper"><a class="block-control" href="javascript:void(0)"></a><h3 class="widget-title heading-title">'.esc_html__('Shop filter', 'yoome').'</h3></div>';
	...
	<div id="main-content" class="<?php echo esc_attr($page_column_class['main_class']); ?>">	
		...
		<?php if ( woocommerce_product_loop() ) : ?>
			...
			<?php
			if( $show_top_filter_widget_area ){
				if( class_exists('WC_Widget_Layered_Nav_Filters') ){
					echo '<div class="ts-active-filters">';
					the_widget('WC_Widget_Layered_Nav_Filters', array('title' => esc_html__('Active filters:', 'yoome')));
					echo '</div>';
				}
				
				$filter_full_width = !($page_column_class['left_sidebar'] || $page_column_class['right_sidebar']);
				if( $filter_full_width ){
				?>
				<div class="vc_row wpb_row vc_row-fluid vc_column-gap-default loading" data-vc-full-width="true" data-vc-full-width-init="true">
				<?php } ?>
