...
function yoome_product_get_availability(){
	global $product;
	$availability = $class = '';
	if ( ! $product->is_in_stock() ) {
		$availability = esc_html__( 'Out of stock', 'yoome' );
	} elseif ( $product->managing_stock() && $product->is_on_backorder( 1 ) ) {
		$availability = esc_html__( 'Available on backorder', 'yoome' );
...
function yoome_template_loop_product_label(){
	global $product, $post;
	$out_of_stock = false;
	$product_stock = yoome_product_get_availability();
	...
	<?php 
	/* New label */
	if( yoome_get_theme_options('ts_product_show_new_label') && !$out_of_stock ){
		...
		if( $num_day <= $num_day_setting ){
			echo '<div><span class="new">'.esc_html(stripslashes(yoome_get_theme_options('ts_product_new_label_text'))).'</span></div>';
		}
	}
	...
	if( !$out_of_stock && $product->is_on_sale() ){
		if( yoome_get_theme_options('ts_show_sale_label_as') != 'text' ){
			...
			echo '<div><span class="onsale">'.esc_html(stripslashes(yoome_get_theme_options('ts_product_sale_label_text'))).'</span></div>';
		}
	}
	
	/* Hot label */
	if( $product->is_featured() && !$out_of_stock ){
		echo '<div><span class="featured">'.esc_html(stripslashes(yoome_get_theme_options('ts_product_feature_label_text'))).'</span></div>';
	}
	
	/* Out of stock */
	if( $out_of_stock ){
		echo '<div><span class="out-of-stock">'.esc_html(stripslashes(yoome_get_theme_options('ts_product_out_of_stock_label_text'))).'</span></div>';
	}
	?>
	</div>
	<?php
