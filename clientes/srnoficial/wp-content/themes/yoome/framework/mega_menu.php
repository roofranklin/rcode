...
if( !class_exists('Yoome_Mega_Menu') ){
	class Yoome_Mega_Menu{
		...
		function save_custom_fields( $menu_id, $menu_item_db_id, $args ){
			...
			if ( isset($_REQUEST['menu-item-ts-bg-color']) && is_array($_REQUEST['menu-item-ts-bg-color']) ) {
				$ts_bg_color = $_REQUEST['menu-item-ts-bg-color'][$menu_item_db_id];
				update_post_meta( $menu_item_db_id, '_menu_item_ts_bg_color', $ts_bg_color );
			}
		...
		function add_data_to_custom_fields( $menu_item ){
			...
			$menu_item->ts_bg_color = get_post_meta( $menu_item->ID, '_menu_item_ts_bg_color', true );
		...
		function register_sidebars(){
			$num_widget = (int) yoome_get_theme_options('ts_menu_num_widget');
			if( $num_widget > 0 ){
				if($num_widget == 1){
		...
		function add_custom_css_for_background(){
			...
			if( $style === false || $style === '0' ){
				...
				$sql = "SELECT * FROM {$wpdb->prefix}postmeta WHERE meta_key = '_menu_item_ts_bg_color' and meta_value <> '';";
				$rows = $wpdb->get_results( $sql );
				
				if( is_array($rows) && !empty($rows) ){
					$header_layouts = apply_filters('yoome_menu_background_color_header_layouts', array('v3'));
					foreach( $rows as $row ){
						$post_id = $row->post_id;
						$bg_color = $row->meta_value;
						
						if( $bg_color ){
							$selector = array();
							foreach( $header_layouts as $header_layout ){
								$selector[] = '.header-'.$header_layout.' header .menu-wrapper nav > ul.menu > li.menu-item-'.$post_id;
							}
							$selector = implode(',', $selector);
							$sub_style = $selector.'{';
							$sub_style .= 'background-color: ' . $bg_color . ';';
							$sub_style .= '}';
			...
			if( $style ){
				wp_add_inline_style('yoome-style', $style);
			}
		}
	}
}
...
if( !class_exists('Yoome_Custom_Mega_Menu') ){
	class Yoome_Custom_Mega_Menu extends Walker_Nav_Menu{
		function __construct(){
		...
		function start_el(&$output, $item, $depth = 0, $args = array(), $current_object_id = 0) {
			...
			if ( ! empty( $item->_invalid ) ) {
				...
				$title = sprintf( esc_html__( '%s (Invalid)','yoome' ), $item->title );
			} elseif ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
				$classes[] = 'pending';
				/* translators: %s: title of menu item in draft status */
				$title = sprintf( esc_html__('%s (Pending)','yoome'), $item->title );
			}
					...
					<dt class="menu-item-handle">
						...
						<span class="item-controls">
							...
							<span class="item-order hide-if-js">
								...
								?>" class="item-move-up"><abbr title="<?php esc_attr_e('Move up','yoome'); ?>">&#8593;</abbr></a>
								|
								<a href="<?php
									echo wp_nonce_url(
										esc_url(
								...
								?>" class="item-move-down"><abbr title="<?php esc_attr_e('Move down','yoome'); ?>">&#8595;</abbr></a>
							</span>
							<a class="item-edit" id="edit-<?php echo esc_attr($item_id); ?>" title="<?php esc_attr_e('Edit Menu Item','yoome'); ?>" href="<?php
							...
							?>"><?php esc_html_e( 'Edit Menu Item','yoome' ); ?></a>
						</span>
					</dt>
				</dl>
				...
				<div class="menu-item-settings wp-clearfix" id="menu-item-settings-<?php echo esc_attr($item_id); ?>">
					...
					<p class="description description-thin">
						<label for="edit-menu-item-title-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'Navigation Label','yoome' ); ?><br />
							<input type="text" id="edit-menu-item-title-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-title" name="menu-item-title[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->title ); ?>" />
						</label>
					</p>
					<p class="description description-thin">
						<label for="edit-menu-item-attr-title-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'Title Attribute','yoome' ); ?><br />
					...
					<p class="field-link-target description">
						<label for="edit-menu-item-target-<?php echo esc_attr($item_id); ?>">
							...
							<?php esc_html_e( 'Open link in a new window/tab','yoome' ); ?>
						</label>
					</p>
					<p class="field-css-classes description description-thin">
						<label for="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'CSS Classes (optional)','yoome' ); ?><br />
							<input type="text" id="edit-menu-item-classes-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-classes" name="menu-item-classes[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( implode(' ', $item->classes ) ); ?>" />
						</label>
					</p>
					<p class="field-xfn description description-thin">
						<label for="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'Link Relationship (XFN)','yoome' ); ?><br />
							<input type="text" id="edit-menu-item-xfn-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-xfn" name="menu-item-xfn[<?php echo esc_attr($item_id); ?>]" value="<?php echo esc_attr( $item->xfn ); ?>" />
						</label>
					</p>
					<p class="field-description description description-wide">
						<label for="edit-menu-item-description-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'Description','yoome' ); ?><br />
							<textarea id="edit-menu-item-description-<?php echo esc_attr($item_id); ?>" class="widefat edit-menu-item-description" rows="3" cols="20" name="menu-item-description[<?php echo esc_attr($item_id); ?>]"><?php echo esc_html( $item->description ); // textarea_escaped ?></textarea>
							<span class="description"><?php esc_html_e('The description will be displayed in the menu if the current theme supports it.','yoome'); ?></span>
						</label>
					</p>        
					<?php
					/*
					...
					<p class="field-ts-sub-label-text description description-thin">
						<label for="edit-menu-item-ts-sub-label-text-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e('Sub Label Text', 'yoome'); ?>
						</label>
					...
					<p class="field-ts-sub-label-bg-color description description-thin">
						<label for="edit-menu-item-ts-sub-label-bg-color-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e('Sub Label Background Color', 'yoome'); ?>
						</label>
					...
					<p class="field-ts-bg-color description description-wide">
						<label for="edit-menu-item-ts-bg-color-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e('Background Color', 'yoome'); ?>
						</label>
						<input value="<?php echo esc_attr($item->ts_bg_color); ?>" type="text" id="edit-menu-item-ts-bg-color-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-ts-bg-color" name="menu-item-ts-bg-color[<?php echo esc_attr($item_id); ?>]" />
						<span class="description"><?php esc_html_e('This option is only used for some specific header layouts', 'yoome'); ?></span>
					</p>
					...
					<p class="field-ts-is-megamenu description description-wide ts-custom-menu ts-active-lv0">
						<label for="edit-menu-item-ts-is-megamenu-<?php echo esc_attr($item_id); ?>">
							...
							<?php esc_html_e('Enable Mega Menu', 'yoome'); ?>
						</label>
					</p>		
					...
					<p class="field-ts-megamenu-column description description-thin ts-custom-menu ts-active-lv0">
						<label for="edit-menu-item-ts-megamenu-column-<?php echo esc_attr($item_id); ?>">
							...
							<?php esc_html_e( 'Columns','yoome' ); ?><br />
							<select id="edit-menu-item-ts-megamenu-column-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-ts-megamenu-column" name="menu-item-ts-megamenu-column[<?php echo esc_attr($item_id); ?>]">
							   <option value="-2" <?php selected(-2, $ts_megamenu_column); ?> ><?php esc_html_e('Stretch row & content', 'yoome') ?></option>
							   <option value="-1" <?php selected(-1, $ts_megamenu_column); ?> ><?php esc_html_e('Stretch row', 'yoome') ?></option>
							   <option value="0" <?php selected(0, $ts_megamenu_column); ?> ><?php esc_html_e('Fullwidth', 'yoome') ?></option>
							   <option value="1" <?php selected(1, $ts_megamenu_column); ?> ><?php esc_html_e('1 column', 'yoome') ?></option>
							   <option value="2" <?php selected(2, $ts_megamenu_column); ?> ><?php esc_html_e('2 columns', 'yoome') ?></option>
							   <option value="3" <?php selected(3, $ts_megamenu_column); ?> ><?php esc_html_e('3 columns', 'yoome') ?></option>
							   <option value="4" <?php selected(4, $ts_megamenu_column); ?> ><?php esc_html_e('4 columns', 'yoome') ?></option>
							</select> 
						</label>
					</p>	
					...
					<?php if( $num_widget > 0):?>
							<p class="field-wide-widget description description-thin ts-custom-menu ts-active-lv0">
								<label for="edit-menu-item-ts-sidebars-<?php echo esc_attr($item_id); ?>">
									<span class="description"><?php esc_html_e('Display A Widget Area','yoome'); ?></span>
									<?php echo Yoome_Mega_Menu::select_sidebar_html($item_id, $num_widget); ?>
								</label>
							</p>
					<?php endif ?>	
					...
					<p class="field-ts-static-html description description-wide ts-custom-menu ts-active-lv0">
						<label for="edit-menu-item-ts-static-html-<?php echo esc_attr($item_id); ?>">
							<?php esc_html_e( 'Static HTML','yoome' ); ?><br />
					...
					<p class="field-ts-thumbnail description description-thin">
						<label>
							<?php 
								...
								esc_html_e( 'Set Thumbnail', 'yoome' ); 
							?>
							<br />
							<a id="<?php echo esc_attr($item_id); ?>" class="ts_mega_menu_upload_image" href="javascript:void(0)" style="display:<?php echo !empty($ts_thumbnail) ? 'none' : 'inline'; ?>;"><?php esc_html_e('Select Image', 'yoome'); ?></a>
							...
							<a id="<?php echo esc_attr($item_id); ?>" class="ts_mega_menu_clear_image" href="javascript:void(0)" style="display:<?php echo !empty($ts_thumbnail) ? 'inline' : 'none'; ?>;"><?php esc_html_e('Remove Image', 'yoome'); ?></a>
					...
					<p class="field-ts-background description description-thin ts-custom-menu ts-active-lv0">
						<label>
							<?php 
								...
								esc_html_e( 'Set Background', 'yoome' ); 
							?>
							<br />
							<a id="<?php echo esc_attr($item_id); ?>" class="ts_mega_menu_upload_image" href="javascript:void(0)" style="display:<?php echo !empty($ts_background) ? 'none' : 'inline'; ?>;"><?php esc_html_e('Select Image', 'yoome'); ?></a>
							<span class="preview-thumbnail-wrapper">
							...
							<a id="<?php echo esc_attr($item_id); ?>" class="ts_mega_menu_clear_image" href="javascript:void(0)" style="display:<?php echo !empty($ts_background) ? 'inline' : 'none'; ?>;"><?php esc_html_e('Remove Image', 'yoome'); ?></a>
					...
					<p class="field-ts-background-repeat description description-thin ts-custom-menu ts-active-lv0">
						<label for="edit-menu-item-ts-background-repeat-<?php echo esc_attr($item_id); ?>">
							...
							<?php esc_html_e( 'Background Repeat', 'yoome' ); ?><br />
							<select id="edit-menu-item-ts-background-repead-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-ts-background-repeat" name="menu-item-ts-background-repeat[<?php echo esc_attr($item_id); ?>]">
							   <option value="no-repeat" <?php selected('no-repeat', $ts_background_repeat); ?> ><?php esc_html_e('No Repeat', 'yoome') ?></option>
							   <option value="repeat" <?php selected('repeat', $ts_background_repeat); ?> ><?php esc_html_e('Repeat', 'yoome') ?></option>
							   <option value="repeat-x" <?php selected('repeat-x', $ts_background_repeat); ?> ><?php esc_html_e('Repeat X', 'yoome') ?></option>
							   <option value="repeat-y" <?php selected('repeat-y', $ts_background_repeat); ?> ><?php esc_html_e('Repeat Y', 'yoome') ?></option>
							</select> 
						</label>
					</p>
					...
					<p class="field-ts-background-position description description-thin ts-custom-menu ts-active-lv0">
						<label for="edit-menu-item-ts-background-position-<?php echo esc_attr($item_id); ?>">
							...
							<?php esc_html_e( 'Background Position', 'yoome' ); ?><br />
							<select id="edit-menu-item-ts-background-position-<?php echo esc_attr($item_id); ?>" class="widefat code edit-menu-item-ts-background-position" name="menu-item-ts-background-position[<?php echo esc_attr($item_id); ?>]">
							   <option value="left-top" <?php selected('left-top', $ts_background_position); ?> ><?php esc_html_e('Left Top', 'yoome') ?></option>
							   <option value="left-bottom" <?php selected('left-bottom', $ts_background_position); ?> ><?php esc_html_e('Left Bottom', 'yoome') ?></option>
							   <option value="left-center" <?php selected('left-center', $ts_background_position); ?> ><?php esc_html_e('Left Center', 'yoome') ?></option>
							   <option value="right-top" <?php selected('right-top', $ts_background_position); ?> ><?php esc_html_e('Right Top', 'yoome') ?></option>
							   <option value="right-bottom" <?php selected('right-bottom', $ts_background_position); ?> ><?php esc_html_e('Right Bottom', 'yoome') ?></option>
							   <option value="right-center" <?php selected('right-center', $ts_background_position); ?> ><?php esc_html_e('Right Center', 'yoome') ?></option>
							   <option value="center-top" <?php selected('center-top', $ts_background_position); ?> ><?php esc_html_e('Center Top', 'yoome') ?></option>
							   <option value="center-bottom" <?php selected('center-bottom', $ts_background_position); ?> ><?php esc_html_e('Center Bottom', 'yoome') ?></option>
							   <option value="center-center" <?php selected('center-center', $ts_background_position); ?> ><?php esc_html_e('Center Center', 'yoome') ?></option>
							</select> 
						</label>
					</p>
					...
					<div class="menu-item-actions description-wide submitbox">
						<?php if( 'custom' != $item->type && $original_title !== false ) : ?>
							<p class="link-to-original">
								<?php printf( esc_html__('Original: %s','yoome'), '<a href="' . esc_url( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
							</p>
						<?php endif; ?>
						...
						); ?>"><?php esc_html_e('Remove','yoome'); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo esc_attr($item_id); ?>" href="<?php echo esc_url( add_query_arg( array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) ) );
							?>#menu-item-settings-<?php echo esc_attr($item_id); ?>"><?php esc_html_e('Cancel','yoome'); ?></a>
					</div>
...
if( !class_exists('Yoome_Walker_Nav_Menu') ){
	class Yoome_Walker_Nav_Menu extends Walker_Nav_Menu{
		...
		function __construct(){
			$this->menu_config['num_widget'] = (int) yoome_get_theme_options('ts_menu_num_widget');
			$this->menu_config['thumb_width'] = (int) yoome_get_theme_options('ts_menu_thumb_width');
			$this->menu_config['thumb_height'] = (int) yoome_get_theme_options('ts_menu_thumb_height');
		}
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ){
			...
			if( $depth === 0 || ($depth > 0 && !$this->parent_megamenu) ){
				...
				$icon_html = '';
				if( $thumbnail_id > 0 ){
					$icon_html = '<span class="menu-icon">'.wp_get_attachment_image( $thumbnail_id, 'yoome_menu_icon_thumb', false).'</span>';
				}
