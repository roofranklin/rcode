...
function yoome_theme_activation(){
	...
	if( is_admin() && 'themes.php' == $pagenow && isset($_GET['activated']) )
	{
		if( get_option( 'woocommerce_single_image_width' ) === false ){
			...
			update_option('woocommerce_thumbnail_cropping_custom_height', 519);
		}
		
		if( get_option( 'yith_woocompare_image_size' ) === false ){
			update_option( 'yith_woocompare_image_size', array( 'width' => '410', 'height' => '519', 'crop' => 1 ) );
		}
	}
}
add_action('admin_init', 'yoome_theme_activation');

/*** Theme Setup ***/
function yoome_theme_setup(){
	...
	/* Translation */
	load_theme_textdomain( 'yoome', get_template_directory() . '/languages' );
	
	/* Register Menu Location */
	register_nav_menus( array(
		'primary' => esc_html__( 'Primary Navigation', 'yoome' ),
	) );
	register_nav_menus( array(
		'vertical' => esc_html__( 'Vertical Navigation', 'yoome' ),
	) );
	register_nav_menus( array(
		'mobile' => esc_html__( 'Mobile Navigation', 'yoome' ),
	) );
}
add_action( 'after_setup_theme', 'yoome_theme_setup');

add_action('init', 'yoome_support_wc_product_gallery_lightbox');
function yoome_support_wc_product_gallery_lightbox(){
...
/*** Add Image Size ***/
function yoome_add_image_size(){
	add_image_size('yoome_menu_icon_thumb', (int) yoome_get_theme_options('ts_menu_thumb_width'), (int) yoome_get_theme_options('ts_menu_thumb_height'), true);
	
	add_image_size('yoome_blog_shortcode_thumb', 690, 460, true);
	add_image_size('yoome_blog_thumb', 1170, 837, true);
	
	add_image_size('yoome_product_cat', 640, 640, true);
}
add_action('init', 'yoome_add_image_size');

add_filter('subcategory_archive_thumbnail_size', 'yoome_product_categories_thumbnail_size');
function yoome_product_categories_thumbnail_size(){
	return 'yoome_product_cat';
}

/*** Get Theme Version ***/
function yoome_get_theme_version(){
	$theme = wp_get_theme();
	if( $theme->parent() ){
...
function yoome_register_scripts(){
	$theme_version = yoome_get_theme_version();
	...
	wp_enqueue_style( 'yoome-reset', get_template_directory_uri() . '/css/reset.css', array(), $theme_version );
	
	wp_enqueue_style( 'yoome-style', get_stylesheet_uri(), array(), $theme_version );
	
	if( !get_option('ts_load_dynamic_style') ){
		wp_enqueue_style( 'yoome-font-color', get_template_directory_uri() . '/css/font-color.css', array(), $theme_version );
	}
	
	if( yoome_load_dokan_style() ){
		wp_enqueue_style( 'yoome-dokan', get_template_directory_uri() . '/css/dokan.css', array(), $theme_version );
	}
	
	if( yoome_get_theme_options('ts_responsive') ){
		wp_enqueue_style( 'yoome-responsive', get_template_directory_uri() . '/css/responsive.css', array(), $theme_version );
	}
	...
	if( yoome_get_theme_options('ts_enable_rtl') ){
		wp_enqueue_style( 'yoome-rtl', get_template_directory_uri() . '/css/rtl.css', array(), $theme_version );
		if( yoome_get_theme_options('ts_responsive') ){
			wp_enqueue_style( 'yoome-rtl-responsive', get_template_directory_uri() . '/css/rtl-responsive.css', array(), $theme_version );
		}
	}
	
	if( yoome_enable_loading_screen() ){
		wp_enqueue_script( 'yoome-loading-screen', get_template_directory_uri() . '/js/loading-screen.js', array('jquery'), $theme_version, false );
		wp_localize_script( 'yoome-loading-screen', 'ts_loading_screen_opt', array('loading_image' => yoome_get_loading_screen_image()) );
	}
	...
	wp_enqueue_script( 'yoome-script', get_template_directory_uri() . '/js/main.js', array('jquery'), $theme_version, true );
	
	if( wp_is_mobile() && yoome_get_theme_options('ts_add_to_cart_effect') == 'fly_to_cart' ){
		yoome_change_theme_options('ts_add_to_cart_effect', '');
	}
	...
	$script_params = array(
		'ajax_url'					=> $ajax_url
		,'sticky_header'			=> (int)(!wp_is_mobile() && yoome_get_theme_options('ts_enable_sticky_header'))
		,'responsive'				=> (int)yoome_get_theme_options('ts_responsive')
		,'ajax_search'				=> (int)yoome_get_theme_options('ts_ajax_search')
		,'show_cart_after_adding'	=> (int)yoome_get_theme_options('ts_show_shopping_cart_after_adding')
		,'add_to_cart_effect'		=> yoome_get_theme_options('ts_add_to_cart_effect')
		,'menu_hover_style'			=> yoome_get_theme_options('ts_menu_hover_style')
		,'shop_loading_type'		=> yoome_get_theme_options('ts_prod_cat_loading_type')
	);
	
	wp_localize_script( 'yoome-script', 'yoome_params', $script_params );
	...
	if( is_singular('product') ){
		wp_enqueue_script( 'yoome-single-product', get_template_directory_uri() . '/js/single-product.js', array('jquery'), $theme_version, true );
		if( yoome_get_theme_options('ts_prod_cloudzoom') ){
			wp_enqueue_script( 'cloud-zoom' );
		}
		if( yoome_get_theme_options('ts_prod_thumbnails_style') == 'vertical' ){
	...
	if( !wp_is_mobile() && yoome_get_theme_options('ts_smooth_scroll') ){
	...
	if( !wp_is_mobile() && yoome_get_theme_options('ts_enable_sticky_header') ){
		wp_enqueue_script( 'jquery-sticky', get_template_directory_uri() . '/js/jquery.sticky.js', array(), $theme_version, true );
	}
	
	if( ( is_tax( get_object_taxonomies( 'product' ) ) || is_post_type_archive('product') ) && yoome_get_theme_options('ts_prod_cat_loading_type') != 'default' ){
		wp_enqueue_script( 'yoome-shop-load-more', get_template_directory_uri() . '/js/shop-load-more.js', array(), $theme_version, true );
	}
	...
	if( !class_exists('ReduxFramework') ){
		wp_enqueue_style( 'yoome-google-fonts', '//fonts.googleapis.com/css?family=Montserrat:300,400,500,600,700' );
	}
	
	/* Custom JS */
	if( $custom_js = yoome_get_theme_options('ts_custom_javascript_code') ){
		wp_add_inline_script( 'yoome-script', stripslashes( trim( $custom_js ) ) );
	}
}
add_action('wp_enqueue_scripts', 'yoome_register_scripts', 1000);

/* Loading Screen */
function yoome_enable_loading_screen(){
	global $post;
	...
	return apply_filters('yoome_enable_loading_screen', $enabled);
}

function yoome_get_loading_screen_image(){
	$theme_options = yoome_get_theme_options();
...
function yoome_get_last_save_theme_options(){
	$transients = get_option('yoome_theme_options-transients', array());
...
function yoome_register_custom_style(){
	...
	if( file_exists( $filename_dir ) ){ 
		wp_enqueue_style('yoome-dynamic-css', $filename, array(), yoome_get_last_save_theme_options());
	}
	else{
		ob_start();
		...
		ob_end_clean();
		wp_add_inline_style( 'yoome-style', $dynamic_css );
	}
}
add_action('wp_enqueue_scripts', 'yoome_register_custom_style', 9999);
...
if( isset($_GET['action']) && $_GET['action'] == 'yith-woocompare-view-table' ){
	add_action('wp_print_styles', 'yoome_add_font_style_to_compare_popup', 1000);
}
function yoome_add_font_style_to_compare_popup(){
	wp_enqueue_style( 'redux-google-fonts-yoome_theme_options' );
	wp_enqueue_style( 'yoome-reset' );
	wp_enqueue_style( 'yoome-style' );
	wp_enqueue_style( 'font-awesome' );
	wp_enqueue_style( 'font-linear-icon' );
	if( yoome_get_theme_options('ts_enable_rtl') ){
		wp_enqueue_style( 'yoome-rtl' );
	}
	wp_enqueue_style( 'yoome-dynamic-css' );
}

/*** Register Back End Scripts ***/
function yoome_register_admin_scripts(){
	$theme_version = yoome_get_theme_version();
	wp_enqueue_media();
	
	wp_enqueue_style( 'font-awesome', get_template_directory_uri() . '/css/font-awesome.css', array(), $theme_version );
	
	wp_enqueue_style( 'yoome-admin-style', get_template_directory_uri() . '/css/admin_style.css', array(), $theme_version );
	
	wp_enqueue_script( 'yoome-admin-script', get_template_directory_uri() . '/js/admin_main.js', array('jquery'), $theme_version, true );
	
	$admin_texts = array(
		'select_images' 			=> esc_html__('Select Images', 'yoome')
		,'use_images' 				=> esc_html__('Use images', 'yoome')
		,'choose_an_image' 			=> esc_html__('Choose an image', 'yoome')
		,'use_image' 				=> esc_html__('Use image', 'yoome')
		,'delete_sidebar_confirm' 	=> esc_html__('Do you want to delete this sidebar?', 'yoome')
		,'delete_sidebar_failed' 	=> esc_html__('Cant delete the sidebar. Please try again!', 'yoome')
	);
	wp_localize_script('yoome-admin-script', 'yoome_admin_texts', $admin_texts);
}
add_action('admin_enqueue_scripts', 'yoome_register_admin_scripts');

/*** Global Page Options ***/
if( !function_exists('yoome_set_global_page_options') ){
	function yoome_set_global_page_options( $page_id = 0 ){
		global $yoome_page_options;
		...
		foreach( $post_custom as $key => $value ){
			if( isset($value[0]) ){
				$yoome_page_options[$key] = $value[0];
			}
		}
		...
		$yoome_page_options = array_merge($default_options, (array) $yoome_page_options);
		return $yoome_page_options;
	}
}

if( !function_exists('yoome_get_page_options') ){
	function yoome_get_page_options( $key = '', $default = '' ){
		global $yoome_page_options;
		if( !$key ){
			return $yoome_page_options;
		}
		else if( isset($yoome_page_options[$key]) ){
			return $yoome_page_options[$key];
		}
		else{
			return $default;
		}
...
if( !function_exists ('yoome_get_vertical_menu_heading') ){
	function yoome_get_vertical_menu_heading(){
		...
		return esc_html__('Shop by category', 'yoome');
	}
}

/*** Get excerpt ***/
if( !function_exists ('yoome_string_limit_words') ){
	function yoome_string_limit_words($string, $word_limit){
...
if( !function_exists ('yoome_the_excerpt_max_words') ){
	function yoome_the_excerpt_max_words( $word_limit = -1, $post = '', $strip_tags = true, $extra_str = '', $echo = true ) {
		if( $post ){
			$excerpt = yoome_get_the_excerpt_by_id($post->ID);
		}
		else{
		...
		if( $word_limit != -1 ){
			$result = yoome_string_limit_words($excerpt, $word_limit);
		}	
		else{
			$result = $excerpt;
		}
...
if( !function_exists('yoome_get_the_excerpt_by_id') ){
	function yoome_get_the_excerpt_by_id( $post_id = 0 )
	{
		global $wpdb;
...
/* Get User Role */
if( !function_exists('yoome_get_user_role') ){
	function yoome_get_user_role( $user_id ){
		global $wpdb;
...
if( !function_exists('yoome_page_layout_columns_class') ){
	function yoome_page_layout_columns_class($page_column, $left_sidebar_name = '', $right_sidebar_name = ''){
		$data = array();
...
/*** Show Page Slider ***/
function yoome_show_page_slider(){
	$page_options = yoome_get_page_options();
...
function yoome_get_portfolio_page_info( $return_url = false ){
	$page_id = yoome_get_theme_options('ts_portfolio_page');
	if( $page_id ){
		if( $return_url ){
...
/*** Breadcrumbs ***/
if( !function_exists('yoome_breadcrumbs') ){
	function yoome_breadcrumbs(){
		$delimiter_char = '&#47;';
		...
		if( !empty( $front_id ) ){
			...
			$home = esc_html__( 'Home', 'yoome' );
		}
		$ar_title = array(
					'search' 		=> esc_html__('Search results for ', 'yoome')
					,'404' 			=> esc_html__('Error 404', 'yoome')
					,'tagged' 		=> esc_html__('Tagged ', 'yoome')
					,'author' 		=> esc_html__('Articles posted by ', 'yoome')
					,'page' 		=> esc_html__('Page', 'yoome')
					);
			...
			elseif( is_search() ){
				...
				if( get_post_type() != 'post' ){
					...
					if( is_singular('ts_portfolio') ){
						$portfolio_page_info = yoome_get_portfolio_page_info();
...
if( !function_exists('yoome_breadcrumbs_title') ){
	function yoome_breadcrumbs_title( $show_breadcrumb = false, $show_page_title = false, $page_title = '', $extra_class_title = '' ){
		$theme_options = yoome_get_theme_options();
		if( $show_breadcrumb || $show_page_title ){
			...
			if( $theme_options['ts_breadcrumb_layout'] != 'v2' ){
				if( $show_page_title ){
...
/*** Pagination ***/
if( !function_exists('yoome_pagination') ){
	function yoome_pagination( $query = null ){
		global $wp_query;
...
/*** Logo ***/
if( !function_exists('yoome_theme_logo') ){
	function yoome_theme_logo(){
		$theme_options = yoome_get_theme_options();
		...
		$logo_image_mobile_menu = is_array($theme_options['ts_logo_mobile_menu'])?$theme_options['ts_logo_mobile_menu']['url']:$theme_options['ts_logo_mobile_menu'];
		...
		if( !$logo_image_mobile_menu ){
			$logo_image_mobile_menu = $logo_image;
		}
		...
		<div class="logo">
			...
			<!-- Mobile menu logo -->
			<?php if( $logo_image_mobile_menu ): ?>
				<img src="<?php echo esc_url($logo_image_mobile_menu); ?>" alt="<?php echo esc_attr($logo_text); ?>" title="<?php echo esc_attr($logo_text); ?>" class="mobile-menu-logo" />
			<?php endif; ?>
...
/*** Pingback URL ***/
add_action('wp_head', 'yoome_pingback_header');
if( !function_exists('yoome_pingback_header') ){
	function yoome_pingback_header(){
...
/*** Favicon ***/
if( !function_exists('yoome_theme_favicon') ){
	function yoome_theme_favicon(){
		if( function_exists('wp_site_icon') && function_exists('has_site_icon') && has_site_icon() ){
			return;
		}
		$favicon_option = yoome_get_theme_options('ts_favicon');
...
/*** Header Template ***/
if( !function_exists('yoome_get_header_template') ){
	function yoome_get_header_template(){
		get_template_part('templates/headers/header', yoome_get_theme_options('ts_header_layout'));
	}
}

if( !function_exists('yoome_get_footer_content') ){
	function yoome_get_footer_content( $footer_block_id = 0 ){
		global $post;
		$args = array(
...
/* Ajax search */
add_action( 'wp_ajax_yoome_ajax_search', 'yoome_ajax_search' );
add_action( 'wp_ajax_nopriv_yoome_ajax_search', 'yoome_ajax_search' );
if( !function_exists('yoome_ajax_search') ){
	function yoome_ajax_search(){
		global $wpdb, $post;
		...
		$num_result = (int)yoome_get_theme_options('ts_ajax_search_number_result');
		$desc_limit_words = (int)yoome_get_theme_options('ts_prod_cat_grid_desc_words');
		...
		if( $results->have_posts() ){
			...
			while( $results->have_posts() ){
				...
				$html .= '<li>';
					...
					$html .= '<div class="ts-wg-meta">';
						$html .= '<a href="'.esc_url($link).'" class="title">'. yoome_search_highlight_string($post->post_title, $search_string) .'</a>';
						$html .= '<div class="description">'. yoome_the_excerpt_max_words($desc_limit_words, '', true, ' ...', false) .'</div>';
						if( $post_type == 'product' ){
			...
			if( isset($results->post_count, $results->found_posts) && $results->found_posts > $results->post_count ){
				$view_all_text = sprintf( esc_html__('View all %d results', 'yoome'), $results->found_posts );
		...
		$return = array();
		$return['html'] = '<p>'.esc_html__('No products were found', 'yoome').'</p>';
		$return['search_string'] = $search_string;
		die( json_encode($return) );
	}
}

if( !function_exists('yoome_search_highlight_string') ){
	function yoome_search_highlight_string($string, $search_string){
		$new_string = '';
...
if( !function_exists('yoome_get_post_comment_count') ){
	function yoome_get_post_comment_count( $post_id = 0 ){
		global $post;
		if( !$post_id ){
			$post_id = $post->ID;
		}
...
if( !function_exists('yoome_post_comment_count') ){
	function yoome_post_comment_count( $post_id = 0 ){
		echo yoome_get_post_comment_count( $post_id );
	}
}

/* Match with ajax search results */
add_filter('woocommerce_get_catalog_ordering_args', 'yoome_woocommerce_get_catalog_ordering_args_filter');
if( !function_exists('yoome_woocommerce_get_catalog_ordering_args_filter') ){
	function yoome_woocommerce_get_catalog_ordering_args_filter( $args ){
		if( is_search() && !isset($_GET['orderby']) && get_option( 'woocommerce_default_catalog_orderby' ) == 'menu_order' 
			&& yoome_get_theme_options('ts_ajax_search') ){
			$args['orderby'] = '';
			$args['order'] = '';
		}
		return $args;
...
if( !function_exists('yoome_get_mailchimp_forms') ){
	function yoome_get_mailchimp_forms(){
		$args = array(
...
/* Add to cart popup */
add_action('wp_footer', 'yoome_add_to_cart_popup_modal');
function yoome_add_to_cart_popup_modal(){
	if( yoome_get_theme_options('ts_add_to_cart_effect') == 'show_popup' ){
	?>
...
add_action('wp_ajax_yoome_load_product_added_to_cart', 'yoome_load_product_added_to_cart' );
add_action('wp_ajax_nopriv_yoome_load_product_added_to_cart', 'yoome_load_product_added_to_cart' );
function yoome_load_product_added_to_cart(){
	if( isset($_POST['product_id']) ){
		...
		?>
		<div class="heading">
			<h6><?php esc_html_e('Product is added to cart', 'yoome'); ?></h6>
		</div>
		<div class="item">
		...
		<div class="action">
			<a href="<?php echo wc_get_cart_url(); ?>" class="button view-cart"><?php esc_html_e('View Cart', 'yoome'); ?></a>
			<a href="<?php echo wc_get_checkout_url(); ?>" class="button checkout"><?php esc_html_e('Checkout', 'yoome'); ?></a>
		</div>
		<?php
		die( ob_get_clean() );
	}
...
/* Support Dokan */
function yoome_load_dokan_style(){
	...
	if( ( function_exists('dokan_is_store_page') && dokan_is_store_page() ) 
		...
		|| apply_filters( 'yoome_forced_load_dokan_style', false ) ){
		return true;	
	}
	return false;
}

add_action('dokan_dashboard_wrap_before', 'yoome_dokan_dashboard_wrap_before', 10, 2);
add_action('dokan_edit_product_wrap_before', 'yoome_dokan_dashboard_wrap_before', 10, 2);
function yoome_dokan_dashboard_wrap_before( $post, $post_id ){
	if( isset( $_GET['product_id'] ) ){
		return;
	}
	yoome_breadcrumbs_title(true, true, get_the_title());
	?>
	<div class="page-container show_breadcrumb_<?php echo yoome_get_theme_options('ts_breadcrumb_layout') ?>">
		<div id="main-content" class="ts-col-24">
	<?php
}

add_action('dokan_dashboard_wrap_after', 'yoome_dokan_dashboard_wrap_after', 10, 2);
add_action('dokan_edit_product_wrap_after', 'yoome_dokan_dashboard_wrap_after', 10, 2);
function yoome_dokan_dashboard_wrap_after( $post, $post_id ){
	if( isset( $_GET['product_id'] ) ){
		return;
	}
	?>
...
add_action( 'tgmpa_register', 'yoome_register_required_plugins' );
function yoome_register_required_plugins(){
    ...
    $plugins = array(
		...
		,array(
            'name'                => 'WooCommerce'
            ,'slug'               => 'woocommerce'
			,'source'             => 'https://downloads.wordpress.org/plugin/woocommerce.5.1.0.zip'
            ,'required'           => true
			,'version'            => '5.1.0'
            ,'external_url'       => ''
        )
		...
		,array(
            'name'                => 'Contact Form 7'
            ,'slug'               => 'contact-form-7'
			,'source'             => 'https://downloads.wordpress.org/plugin/contact-form-7.5.4.zip'
            ,'required'           => false
			,'version'            => '5.4'
            ,'external_url'       => ''
        )
		,array(
			...
			,'source'             => 'https://downloads.wordpress.org/plugin/mailchimp-for-wp.4.8.3.zip'
            ,'required'           => false
			,'version'            => '4.8.3'
            ,'external_url'       => ''
        )
		,array(
			...
			,'source'             => 'https://downloads.wordpress.org/plugin/yith-woocommerce-wishlist.3.0.20.zip'
            ,'required'           => false
			,'version'            => '3.0.20'
            ,'external_url'       => ''
        )
		,array(
			...
			,'source'             => 'https://downloads.wordpress.org/plugin/yith-woocommerce-compare.2.4.6.zip'
            ,'required'           => false
			,'version'            => '2.4.6'
            ,'external_url'       => ''
        )
