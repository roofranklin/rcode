	...
	class TS_Recent_Comments_Widget extends WP_Widget {
							...
							<li>
								<div class="comment-meta">
									...
									<div class="meta">
										...
										<?php if( $show_author ): ?>
										<span class="author">
											<?php esc_html_e('Post by ', 'themesky'); ?><a href="<?php echo (get_comment_author_url() == '')?'javascript: void(0)':get_comment_author_url(); ?>" rel="external nofollow"><?php echo esc_html($comment->comment_author); ?></a>
										</span>
										<?php endif; ?>
		
									</div>
								</div>
								<?php if( $show_comment && function_exists('yoome_string_limit_words') ): ?>
								<blockquote class="comment-body"><?php echo yoome_string_limit_words(wp_strip_all_tags(get_comment_text()), 15); ?></blockquote>
								<?php endif; ?>
							</li>
		...
		?>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_avatar'); ?>"><?php esc_html_e('Show avatar', 'themesky'); ?></label>
			</p>
