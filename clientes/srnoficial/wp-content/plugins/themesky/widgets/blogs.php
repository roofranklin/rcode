...
if( !class_exists('TS_Blogs_Widget') ){
	class TS_Blogs_Widget extends WP_Widget {
		...
		function widget( $args, $instance ) {
			...
			$show_comment 		= empty($instance['show_comment']) || !function_exists('yoome_post_comment_count')?0:$instance['show_comment'];
			$show_like 			= empty($instance['show_like'])?0:$instance['show_like'];
							...
							<li>
											...
											<figure>
												...
												if( $post_format == 'gallery' ){
													...
													foreach( $gallery_ids as $gallery_id ){
														echo wp_get_attachment_image( $gallery_id, 'yoome_blog_shortcode_thumb' );
													}
												}
												
												if( $post_format === false || $post_format == 'standard' ){
													if( has_post_thumbnail() ){
														the_post_thumbnail('yoome_blog_shortcode_thumb'); 
													}
												}
												?>
											</figure>
								...
								<?php if( $post_format != 'quote' ): ?>
									...
									<header>
										...
										<?php if( $show_author ): ?>
										<span class="author">
											<?php esc_html_e('Post by ', 'themesky'); ?><?php the_author_posts_link(); ?>
										</span>
										<?php endif; ?>
									...
									<?php if( $show_excerpt && function_exists('yoome_the_excerpt_max_words') ): ?>
										<div class="excerpt">
											<?php yoome_the_excerpt_max_words($excerpt_words, $post); ?>
										</div>
									<?php endif; ?>
		...
		?>
			...
			<p>
				...
				<label for="<?php echo $this->get_field_id('show_comment'); ?>"><?php esc_html_e('Show post comment', 'themesky'); ?></label>
			</p>
			
			<p>
				<input type="checkbox" id="<?php echo $this->get_field_id('show_like'); ?>" name="<?php echo $this->get_field_name('show_like'); ?>" value="1" <?php echo ($instance['show_like'])?'checked':''; ?> />
				<label for="<?php echo $this->get_field_id('show_like'); ?>"><?php esc_html_e('Show post like', 'themesky'); ?></label>
			</p>
