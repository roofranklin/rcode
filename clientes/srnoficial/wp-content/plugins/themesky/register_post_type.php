...
if( !class_exists('TS_Portfolios') ){
	class TS_Portfolios{
		...
		function __construct(){
			...
			$this->thumb_size_array = array(600, 600);
			$this->add_image_size();
...
if( !class_exists('TS_Team_Members') ){
	class TS_Team_Members{
		...
		function __construct(){
			...
			$this->thumb_size_array = array(400, 400);
			$this->add_image_size();
...
if( !class_exists('TS_Testimonials') ){
	class TS_Testimonials{
		...
		function __construct(){
			...
			$this->thumb_size_array = array(100, 100);
			$this->add_image_size();
...
if( !class_exists('TS_Logos') ){
	class TS_Logos{
		...
		function __construct(){
			...
			$logo_width = isset($size_options['size']['width'])?$size_options['size']['width']:230;
			$logo_height = isset($size_options['size']['height'])?$size_options['size']['height']:90;
		...
		function setting_page_content(){
			$options_default = array(
							...
							,'responsive' => array(
								'break_point'	=> array(0, 300, 600, 850, 1170)
...
if( !class_exists('TS_Product_Brands') ){
	class TS_Product_Brands{
		...
		function register_taxonomy(){
			...
			register_taxonomy( 'ts_product_brand',
				array( 'product' ),
				array(
					'hierarchical'           => true
